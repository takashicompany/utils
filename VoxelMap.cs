// takashicompany.Unity.VoxelMap  ― 完全版（コンパイル確認済み）
// -----------------------------------------------------------------------------
// * 2D トーラス型マップ（上下左右ループ）
// * シームレス Perlin fBm（6 オクターブ）+ ランダム回転で格子アーティファクト除去
// * 海はフラット、高度は陸地のみ 0‑1
// * [R] キーで再生成
// -----------------------------------------------------------------------------
...
namespace takashicompany.Unity
{
    public class VoxelMap : MonoBehaviour
    {
        ...
        [Range(0.5f,10f)] public float frequency = 2.5f; // 大陸スケール
        [Range(1,8)] public int octaves = 6;              // オクターブ数
        public float persistence = 0.45f;                 // 振幅減衰
        public int seed = 1337;
        [Range(0,3)][Tooltip("陸地の平滑化パス数")]
        ...
        public void Generate()
        {
            ...
            for(int x=0;x<width;x++)
            {
                ...
                {
                    float h=heights[x,z];
                    tex.SetPixel(x,z, Color.Lerp(new Color(0.12f,0.55f,0.15f), Color.white, Mathf.Pow(h,1.7f)) );
                }
                else
                {
                    tex.SetPixel(x,z, new Color(0f,0.35f,0.75f));
                }
            }
            tex.Apply();
        }
        ...
        {
            if(tex==null) return;
            float scale = Mathf.Min(Screen.width/(float)width, Screen.height/(float)height)*0.9f;
            float w=width*scale, h=height*scale;
            GUI.DrawTexture(new Rect((Screen.width-w)/2f,10,w,h), tex, ScaleMode.ScaleToFit,false);
            GUI.Label(new Rect(10,Screen.height-25,350,20), "[R] regenerate | seamless torus map" );
        }
    ...
    //   2D Perlin を 4 組合成し、[-1,1] を返す
    // ----------------------------------------------------------------------
    internal sealed class SeamlessNoise
    {
        private readonly float[] off = new float[8];
        public SeamlessNoise(int seed)
        {
            var rng=new System.Random(seed);
            for(int i=0;i<off.Length;i++) off[i]=(float)rng.NextDouble()*512f;
        }
        public double Evaluate(double x,double y,double z,double w)
        {
            ...
            return (n1+n2+n3+n4)*0.5f - 1f; // 0‑4 → [-1,1]
        }
    }
}
